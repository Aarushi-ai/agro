...
(function () {
  ...
  const IMAGE_MAP = {
    ...
    "seaweed-liquid": "assets/images/products/Powder Seaweed Extract Fertilizer.jpg",
  ...
  function imgHtml(p) {
    ...
    if (src) return `<img src="${src}" alt="${p.name}" loading="lazy" style="width:100%;height:100%;object-fit:cover"/>`;
  ...
  function cardHtml(p) {
    ...
    const specs = p.specTable.slice(0, 3).map((r) => `<div class="product-spec-row"><span>${r[0]}</span><span>${r[1]}</span></div>`).join("");
    return `<article class="product-card cat-product-card" data-categories="${p.category.join(" ")}">
      <div class="product-img">${imgHtml(p)}${p.discount ? `<span class="product-discount-pill">-${p.discount}%</span>` : ""}<span class="product-form-pill">${p.form}</span></div>
      <div class="product-info">
        <div class="cat-badges">${badgeHtml}</div>
        <h3 class="product-name">${p.name}</h3>
        <p class="product-tagline">${p.tagline}</p>
        <p class="product-key-spec">${p.keySpec}</p>
  ...
  function initHomePreview() {
    ...
    if (!grid) return;
    const featured = ["vanchetan-davya", "energy-balls", "seaweed-liquid", "potassium-humate", "humic-acid-fco", "combo-solution"];
    grid.innerHTML = featured.map((id) => cardHtml(PRODUCTS.find((p) => p.id === id))).join("");
  }
